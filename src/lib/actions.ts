...
function generateQrCodeUrl(articleNo: string) {
    // In a real application, the base URL should come from an environment variable.
    const baseUrl = 'https://app.example.com';
    const productUrl = `${baseUrl}/products/${articleNo}`;
    return `https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${encodeURIComponent(productUrl)}`;
}
...
export async function addProductAction(data: FormData) {
  ...
  try {
    const qrCode = generateQrCodeUrl(validatedFields.data.articleNo);
    await addProduct({ ...validatedFields.data, qrCode });
...
export async function updateProductAction(id: string, data: FormData) {
  ...
  try {
    ...
    const qrCode = generateQrCodeUrl(validatedFields.data.articleNo);
    await updateProduct(id, { ...validatedFields.data, qrCode });
    revalidatePath('/admin/products');
    revalidatePath(`/admin/products/edit/${id}`);
    revalidatePath(`/products/${validatedFields.data.articleNo}`);
    revalidatePath('/admin/fabrics');
